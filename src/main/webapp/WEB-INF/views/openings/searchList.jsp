...
<body>
	...
	<div class="container py-4">
		<form id="form_search" action="searchList.wow?curPage=1" method="post">
			<!-- 폼 액션 서브밋 -->
			<!-- 38 글작성 버튼생성 -->
			...
			<table class="table">
				...
				<tbody>
					<c:forEach items="${searchList}" var="search">
						<tr>
							<td>${search.boNo }</td>
							<td><a href="openingsView.wow?boNo=${search.boNo }">${search.boTitle }</a></td>
							<td>${search.boPos}</td>
							<td>${search.boHit }</td>
							<td>${search.boRegDate }</td>
							<td>${search.boWriter }</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
			...
			<div class="row text-center" style="width: 100%">
				<div style="width: 50%; float: none; margin: 0 auto">
					...
					<select name="searchCate">
						...
						placeholder="검색어를 입력하세요""> <input type="submit" value="검색"><input type="button" value="초기화" onclick="location.href='searchList.wow'">
					<br>
		...
		<script type="text/javascript">
			function f_ck(tag) {
				...
				$("#form_search").attr('action', "searchList.wow?curPage="+ $(tag).attr('data-page'));
				$("#form_search").submit();
			}
			// 레코드 수, 분류 셀렉트
